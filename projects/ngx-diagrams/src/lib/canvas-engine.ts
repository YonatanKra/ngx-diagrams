...
> extends BaseObserver<L> {
	...
	constructor(options: CanvasEngineOptions = {}) {
		super();
		this.model = null;
		//  typescript issue inheriting generic models
		this.eventBus = new ActionEventBus(this as any);
		this.stateMachine = new StateMachine(this as any);
	...
	registerFactoryBank(factory: FactoryBank) {
		factory.registerListener({
			factoryAdded: event => {
				event.factory.setDiagramEngine(this as any);
			},
	...
	getFactoryForLayer<F extends AbstractAngularFactory<LayerModel>>(layer: LayerModel | string) {
		if (typeof layer === 'string') {
			return this.layerFactories.getFactory<F>(layer);
		}
		return this.layerFactories.getFactory<F>(layer.getType());
	}
